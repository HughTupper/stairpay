...
const londonPostcodes = [
  'SW1A 1AA', 'W1A 0AX', 'E1 6AN', 'N1 9AG', 'SE1 7PB',
  'NW1 4RY', 'EC1A 1BB', 'WC2N 5DU', 'E2 9AN', 'SW7 2AZ',
]

const streetNames = [
  'High Street', 'Station Road', 'Church Lane', 'Mill Lane', 'Park Road',
  'London Road', 'Queens Road', 'Victoria Street', 'Manor Road', 'Green Lane',
]

const firstNames = [
  'James', 'Mary', 'John', 'Patricia', 'Robert', 'Jennifer', 'Michael', 'Linda',
  'William', 'Elizabeth', 'David', 'Barbara', 'Richard', 'Susan', 'Joseph', 'Jessica',
  'Thomas', 'Sarah', 'Charles', 'Karen', 'Daniel', 'Emily', 'Matthew', 'Sophie',
]
      ...
      for (let i = 0; i < tenantCount; i++) {
        ...
        const { data, error } = await supabase
          ...
          .insert({
            ...
            email: `tenant${i}@${org.name.toLowerCase().replace(/\s+/g, '')}.com`,
      ...
      for (let i = 0; i < applicationCount; i++) {
        ...
        if (status === 'approved' || status === 'completed') {
          ...
          if (status === 'completed') {
            ...
            completedDate.setDate(completedDate.getDate() + randomNumber(30, 90))
          }
        }
        ...
        await supabase.from('staircasing_applications').insert({
          ...
          notes: status === 'rejected' ? 'Insufficient funds verification' : null,
        })
      }
